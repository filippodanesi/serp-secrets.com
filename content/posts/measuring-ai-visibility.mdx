...
Underneath the sampling issues sits a structural one. A brand can be retrieved and not cited. In [Pages Get Retrieved. Chunks Get Cited.](https://www.serp-secrets.com/blog/pages-retrieved-chunks-cited/) I worked through the two-gate model: pages enter a candidate pool, then passages get selected from inside it. Most visibility tools observe only the final answer, which is the output of gate two. They cannot see the pages that entered the pool and lost.
...
There is a shorter version of all of this. In [The Great Decoupling](https://www.serp-secrets.com/blog/the-great-decoupling/) the lesson was that impressions stopped predicting clicks. The lesson here is the same shape: a visibility score does not predict revenue, and a chart that implies it does will be believed.
