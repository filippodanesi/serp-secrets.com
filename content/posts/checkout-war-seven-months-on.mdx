...
That Google and OpenAI had formalized competing rails for agent-driven purchase, and that merchants would have to decide which to build for. In [Checkout Is the New Battleground](https://www.serp-secrets.com/blog/checkout-new-battleground/) I laid out Google's Universal Commerce Protocol against OpenAI's Agentic Commerce Protocol, argued that product catalogs built for keyword search would not survive conversational reasoning, and closed by saying the protocol wars were not about which company won but which standard became the default rails.
...
This is the same composition change I described in [GEO Grew Up](https://www.serp-secrets.com/blog/geo-grew-up/), and it means the same thing here. The early cohort learned the answer and stopped searching. A much larger mainstream cohort is arriving now and starting from zero.
